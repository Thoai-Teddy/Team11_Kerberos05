...
// ======= ĐỊNH NGHĨA CÁC FLAG/OPTION =========

// Các bit cho Options trong yêu cầu
constexpr uint32_t KDC_OPT_FORWARDABLE = 1 << 1;
constexpr uint32_t KDC_OPT_PROXIABLE = 1 << 2;
constexpr uint32_t KDC_OPT_RENEWABLE = 1 << 3;
constexpr uint32_t KDC_OPT_ENC_TKT_IN_SKEY = 1 << 8;

// ===== Ticket Flags (RFC 4120) =====
constexpr uint32_t FLAG_FORWARDABLE = 1 << 0;
constexpr uint32_t FLAG_FORWARDED = 1 << 1;
constexpr uint32_t FLAG_PROXIABLE = 1 << 2;
constexpr uint32_t FLAG_PROXY = 1 << 3;
constexpr uint32_t FLAG_MAY_POSTDATE = 1 << 4;
constexpr uint32_t FLAG_POSTDATED = 1 << 5;
constexpr uint32_t FLAG_INVALID = 1 << 6;
constexpr uint32_t FLAG_RENEWABLE = 1 << 7;
constexpr uint32_t FLAG_INITIAL = 1 << 8;
constexpr uint32_t FLAG_PRE_AUTHENT = 1 << 9;
constexpr uint32_t FLAG_HW_AUTHENT = 1 << 10;
constexpr uint32_t FLAG_TRANSITED_POLICY_CHECKED = 1 << 11;
constexpr uint32_t FLAG_OK_AS_DELEGATE = 1 << 12;
...
//=============== Step 6===============:
uint32_t parseFlags(const std::string& flagsStr) {
    return std::bitset<32>(flagsStr).to_ulong();
}
std::string flagsToString(uint32_t flags) {
    return std::bitset<32>(flags).to_string();
}
ServiceTicket createServiceTicket(const std::string& clientID, const std::string& flagsStr, const std::string& sessionKey,
    ...
    //ticket.flags = parseFlags(flagsStr);
    ticket.flags = flagsStr;
    ...
    ticket.isDelegationAllowed = false;
    ticket.isAccessRestricted = false;
...
std::string createSubkey(const std::string& key, const std::string& data) { // data có thể là 1 dữ liệu bất kỳ, ở đây sẽ chọn data là TS2, key là Kcv
...
// ========== HÀM TẠO OPTIONS TỪ CLIENT ==========
uint32_t createKerberosOptions(bool forwardable, bool proxiable, bool renewable) {
    uint32_t options = 0;
    if (forwardable) options |= KDC_OPT_FORWARDABLE;
    if (proxiable)   options |= KDC_OPT_PROXIABLE;
    if (renewable)   options |= KDC_OPT_RENEWABLE;
    return options;
}

// ========== HÀM TẠO FLAGS TRONG TICKET ==========
uint32_t createTicketFlags(bool forwardable, bool forwarded, bool renewable, bool initial, bool pre_auth) {
    uint32_t flags = 0;
    if (forwardable) flags |= FLAG_FORWARDABLE;
    if (forwarded)   flags |= FLAG_FORWARDED;
    if (renewable)   flags |= FLAG_RENEWABLE;
    if (initial)     flags |= FLAG_INITIAL;
    if (pre_auth)    flags |= FLAG_PRE_AUTHENT;
    return flags;
}
// Kiểm tra xem 1 flag cụ thể có được bật không
bool hasFlag(uint32_t flags, uint32_t flagToCheck) {
    return (flags & flagToCheck) != 0;
}
bool handleTicketFlags(ServiceTicket& ticket) {
    time_t currentTime = time(nullptr);

    // ----- Kiểm tra hợp lệ -----
    if (hasFlag(ticket.flags, FLAG_INVALID)) {
        std::cerr << "[❌] Ticket is marked INVALID — rejecting access.\n";
        return false;
    }

    if (hasFlag(ticket.flags, FLAG_POSTDATED)) {
        if (currentTime < ticket.startTime) {
            std::cerr << "[❌] Ticket is POSTDATED but not yet valid — rejecting.\n";
            return false;
        }
    }

    if (currentTime > ticket.endTime) {
        std::cerr << "[❌] Ticket expired — rejecting access.\n";
        return false;
    }

    // ----- Ghi nhận các Flag phụ trợ -----
    if (hasFlag(ticket.flags, FLAG_FORWARDED)) {
        std::cout << "[ℹ️] Ticket was FORWARDED — log this for auditing.\n";
    }

    if (hasFlag(ticket.flags, FLAG_PROXY)) {
        std::cout << "[⚠️] Ticket is a PROXY — applying access restrictions.\n";
        ticket.isAccessRestricted = true;
    }

    if (hasFlag(ticket.flags, FLAG_OK_AS_DELEGATE)) {
        std::cout << "[✅] OK-AS-DELEGATE is set — allowing delegation.\n";
        ticket.isDelegationAllowed = true;
    }

    if (hasFlag(ticket.flags, FLAG_HW_AUTHENT)) {
        std::cout << "[🔒] Client used hardware authentication — trust increased.\n";
    }

    if (hasFlag(ticket.flags, FLAG_INITIAL)) {
        std::cout << "[ℹ️] Ticket obtained from AS directly — initial authentication.\n";
    }

    return true; // Nếu không bị từ chối ở trên
}
